...
<body style="background-color: #f4f4f4">
	<div class="container">
		...
		<div class="lj">
			<ol class="breadcrumb row">
				...
				<li><a href="http://localhost:8080/itjl/bk">博客</a></li>
				<li><a href="#">${bk.bk_bt}</a></li>
			</ol>
		</div>
		<div class="fgh"></div>
		<!--        主体-->
		<div class="zt">
			<div class="zt_l">
				<div class="zt_l_body">
					<div hidden="hidden" id="user_id">${bk.user.user_id}</div>
					<!--                    文章主体-->
					<div class="wzzt">
						<div hidden="hidden" id="user_id">${bk.user.user_id}</div>
						<div hidden="hidden" id="bk_id">${bk.bk_id}</div>
						<div hidden="hidden" id="bk_tjl">${bk.bk_tjl}</div>
						<div class="wzzt_top_bt">
							<div class="wzzt_top_bt">
								<b><h3>${bk.bk_bt}</h3></b>
							</div>
							<div class="wzzt_top_xx hs row">
								<span class="col-sm-10">${ bk.lb.lb_name}&nbsp;&nbsp;&nbsp;<span>标签:&nbsp;&nbsp;${bk.bk_bq}</span>&nbsp;&nbsp;&nbsp;<span>${date}&nbsp;&nbsp;来自${bk.user.user_name}</span></span>
								<span class="col-sm-2"><a href="" id="sctext" onclick="sc()">收藏</a>&nbsp;&nbsp;&nbsp;&nbsp;<a
									href="" data-toggle="modal" data-target="#cjxz">举报</a></span>
							</div>
						</div>
						<hr>
						<div class="wzzt_nr">
							<div class="wzzt_nr_zt">${bk.bk_nr}</div>
							<div class="wzzt_foot text-left hs">
								版权声明：<span>本文为博主原创文章，未经博主允许不得转载。</span>
							</div>

							<div class="tj text-center" onclick="tjclick()">推荐</div>
						</div>
						<div class="wzzt_an ">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form role="form">
						<div class="row">
							<div class="col-sm-12">
								<div class="form-group">举报对象:${bk.user.user_name}</div>
							</div>
						</div>

						<div class="row">
							<div class="col-sm-12">
								<div class="form-group">举报文章:${bk.bk_bt}</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-sm-12">
								<div class="form-group">
									举报理由: <select class="form-control">
	...
	<script type="text/javascript">
		...
		var bkid = $("#bk_id").text();
		var bk_tjl = $("#bk_tjl").text();

		function tjclick() {
			$.ajax({
				type : "post",
				dataType : "json",
				url : "http://localhost:8080/itjl/addtj?bkid=" + bkid
						+ "&bk_tjl=" + bk_tjl,
				success : function(data) {
					bk_tjl = data;
					alert("成功添加推荐量！");
		...
		var sctext = $("#sctext").text();
		
		function sc() {
			alert("博客已托管于github，暂时关闭博客收藏");
			$.ajax({
				type : "post",
				dataType : "json",
				url : "http://localhost:8080/itjl/scbk?bkid="+bkid+"&sctext="+sctext,
				success : function(data) {
					alert(data);
				}
			});
